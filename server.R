...
shinyServer(function(input, output, session) {
      ...
      output$indeed_pic1 <- renderImage({
            return(list(src = "www/indeed_logo.png", contentType = "www/png", alt = "indeed_logo", width = 200, height = "50px", align = "left"))
            }, 
      deleteFile = FALSE)
      ...
      dist_link <- reactive({
            ...
            str_replace_all(paste(indeed_url,job_title() %>% str_trim() %>% str_replace(pattern = " ", replacement = "+"),"+", paste(searchLink()[with(searchLink(),which( salary == salary_filter() & jt == jobtype_filter() & locs == locs_filter() & comps == comps_filter() & exper == exps_filter())),c(2,4,6,8,10)] ,collapse = ""),sep=""), pattern = "ALL", replacement = "") %>% str_replace_all(pattern = " \\(.*\\)", replacement = "")
      })
      ...
      jobs <- eventReactive(input$act_search,{
            ...
            # for shiinyapp.io demo
            # dt <- lapply(links, form_table) %>% rbindlist()
      ...
      output$position_leaflet_locs <- renderLeaflet({
            loc_data <- jobs() %>% select(position, company, lat, lng)
            ...
            loc_data <- rbind(loc_data,data.frame(position="You",company="Your Current IP Address", lat=input$lat, lng=input$long))
            loc_data %>% leaflet() %>% addTiles() %>% addCircleMarkers(lng=~lng, lat=~lat,clusterOptions=markerClusterOptions(),popup = ~paste("Position: ", position,", Company: ",company,sep=""),color = c(rep("blue",length=nrow(loc_data)-1),"red"))
      })
